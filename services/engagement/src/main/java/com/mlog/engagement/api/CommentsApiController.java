...
public class CommentsApiController implements CommentsApi {
    ...
    private final CommentsApiDelegate delegate;

    public CommentsApiController(@Autowired(required = false) CommentsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new CommentsApiDelegate() {});
    }

    @Override
    public CommentsApiDelegate getDelegate() {
        return delegate;
    }
