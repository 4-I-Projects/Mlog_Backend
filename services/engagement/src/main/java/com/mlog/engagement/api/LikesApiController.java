...
public class LikesApiController implements LikesApi {
    ...
    private final LikesApiDelegate delegate;

    public LikesApiController(@Autowired(required = false) LikesApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new LikesApiDelegate() {});
    }

    @Override
    public LikesApiDelegate getDelegate() {
        return delegate;
    }
